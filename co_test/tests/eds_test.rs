...
mod eds_tests {
    ...
    use std::panic;
    use co_test::util::SAMPLE_EDS_PATH;

    fn build_od() -> ObjectDirectory {
        let content = std::fs::read_to_string(SAMPLE_EDS_PATH).expect("Failed to read EDS file");
        ObjectDirectory::new(2, &content).expect("Errors in creating an object directory")
    }
